      ...
      if let envReferenceImageDirectory = envReferenceImageDirectory {
        for suffix in suffixes {
          ...
          if viewOrLayer.isKind(of: UIView.self) {
            do {
              try compareSnapshot(of: viewOrLayer as! UIView, referenceImagesDirectory: referenceImagesDirectory, identifier: identifier, pixelTolerance: pixelTolerance, tolerance: tolerance)
            ...
            do {
              try compareSnapshot(of: viewOrLayer as! CALayer, referenceImagesDirectory: referenceImagesDirectory, identifier: identifier, pixelTolerance: pixelTolerance, tolerance: tolerance)
              comparisonSuccess = true
            } catch let error1 as NSError {
              error = error1
              comparisonSuccess = false
...
#else
public extension FBSnapshotTestCase {
  public func FBSnapshotVerifyView(view: UIView, identifier: String = "", suffixes: NSOrderedSet = FBSnapshotTestCaseDefaultSuffixes(), pixelTolerance: CGFloat = 0, tolerance: CGFloat = 0, file: StaticString = #file, line: UInt = #line) {
  FBSnapshotVerifyViewOrLayer(view, identifier: identifier, suffixes: suffixes, pixelTolerance: pixelTolerance, tolerance: tolerance, file: file, line: line)
  }

  public func FBSnapshotVerifyLayer(layer: CALayer, identifier: String = "", suffixes: NSOrderedSet = FBSnapshotTestCaseDefaultSuffixes(), pixelTolerance: CGFloat = 0, tolerance: CGFloat = 0, file: StaticString = #file, line: UInt = #line) {
  FBSnapshotVerifyViewOrLayer(layer, identifier: identifier, suffixes: suffixes, pixelTolerance: pixelTolerance, tolerance: tolerance file: file, line: line)
  }

  private func FBSnapshotVerifyViewOrLayer(viewOrLayer: AnyObject, identifier: String = "", suffixes: NSOrderedSet = FBSnapshotTestCaseDefaultSuffixes(), pixelTolerance: CGFloat = 0, tolerance: CGFloat = 0, file: StaticString = #file, line: UInt = #line) {
    ...
    if let envReferenceImageDirectory = envReferenceImageDirectory {
      for suffix in suffixes {
        ...
        if viewOrLayer.isKindOfClass(UIView) {
          do {
  try compareSnapshotOfView(viewOrLayer as! UIView, referenceImagesDirectory: referenceImagesDirectory, identifier: identifier, pixelTolerance: pixelTolerance, tolerance: tolerance)
            comparisonSuccess = true
          } catch let error1 as NSError {
            error = error1
            comparisonSuccess = false
          ...
          do {
  try compareSnapshotOfLayer(viewOrLayer as! CALayer, referenceImagesDirectory: referenceImagesDirectory, identifier: identifier, pixelTolerance: pixelTolerance, tolerance: tolerance)
            comparisonSuccess = true
          } catch let error1 as NSError {
            error = error1
            comparisonSuccess = false
